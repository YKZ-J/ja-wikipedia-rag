...
Wikipedia 日本語ダンプを取得し、テキストを Parquet 化 → GTE-multilingual-base で 768次元ベクトル生成 → ローカル Supabase (`documents` テーブル) に投入 → IVFFlat インデックスを張るまでの完全手順。
...
| Transformers | 最新       | GTE-multilingual-base の埋め込み生成 |
...
Embedding 生成は Transformers + PyTorch で実行する。
...
```bash
export EMBEDDING_MODEL=Alibaba-NLP/gte-multilingual-base
```
...
チャンク分割（chunk-size件）
    │
    ▼  asyncio + Transformers (workers個並列)
GTE-multilingual-base で埋め込み生成
  → 768次元ベクトル
    │
...
GTE ではプレフィックス指定は不要。文書とクエリの両方を同一モデルで埋め込み生成する。

### 処理速度の目安（Mac mini M4 / 並列実行時）
...
```bash
tar czf parquet_output_$(date +%Y%m%d).tar.gz parquet_output
```
...
# Parquet 複元
tar xzf parquet_output_YYYYMMDD.tar.gz
```
