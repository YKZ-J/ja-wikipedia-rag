...
- サーバー: Python + FastMCP
- LLM: llama.cpp（ローカル）
- Embedding: Transformers (GTE-multilingual-base)
...
4. Python が llama.cpp / Transformers / DB を呼び出して結果を返す
...
- `llama-cpp-python`
  - `.gguf` モデルをローカル推論
- `transformers`, `torch`
  - GTE-multilingual-base による埋め込み生成
- `asyncpg`
  - PostgreSQL 非同期アクセス
- `python-dotenv`
  - `.env.local` から設定ロード
...
2. 埋め込みモデルでベクトル化（Transformers + GTE）
...
- Python MCP が起動していない
- モデルファイルまたは Python 環境が不正
- 環境変数不足

対策:

1. MODEL_PATH と Python 依存（torch/transformers）の確認
...
| `ask_wiki_rag_report`        | `kb ask-wiki-report` | RAG実測レポート生成            |
| `create_wiki_rag_comparison` | `kb compare-wiki`    | RAG比較記事生成                |
| `arange_blog`                | `kb arange-blog`     | 記事の所定箇所へ引用挿入       |
...
askWikiRagReport(query, 3); // ask_wiki_rag_report ツール呼び出し
createWikiRagComparison(query, title, tags); // create_wiki_rag_comparison ツール呼び出し
// ...
```
...
_retrieve_rag_docs(query, db_url)
 │  ├── Transformers GTE-multilingual-base でクエリ埋め込み生成
...
| 処理                                       | 時間             |
| ------------------------------------------ | ---------------- |
| クエリ埋め込み生成 (GTE-multilingual-base) | ハードウェア依存 |
